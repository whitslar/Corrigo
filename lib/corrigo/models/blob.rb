  ...
  class Blob
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Corrigo::Blob` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Corrigo::Blob`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@body.nil? && @body !~ pattern
        invalid_properties.push("invalid value for \"body\", must conform to the pattern #{pattern}.")
      end
    ...
    def valid?
      return false if !@body.nil? && @body !~ Regexp.new(/^(?:[A-Za-z0-9+\/]{4})*(?:[A-Za-z0-9+\/]{2}==|[A-Za-z0-9+\/]{3}=)?$/)
      true
    end
    ...
    def body=(body)
      pattern = Regexp.new(/^(?:[A-Za-z0-9+\/]{4})*(?:[A-Za-z0-9+\/]{2}==|[A-Za-z0-9+\/]{3}=)?$/)
      if !body.nil? && body !~ pattern
        fail ArgumentError, "invalid value for \"body\", must conform to the pattern #{pattern}."
      end
    ...
    def hash
      [id, perform_deletion, actor_id, actor_type_id, body, file_name, type_id].hash
    end
