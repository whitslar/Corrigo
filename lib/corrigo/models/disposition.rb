  ...
  class Disposition
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Corrigo::Disposition` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Corrigo::Disposition`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'on_create')
        self.on_create = attributes[:'on_create']
      end

      if attributes.key?(:'on_complete')
        self.on_complete = attributes[:'on_complete']
      end

      if attributes.key?(:'on_cancel')
        self.on_cancel = attributes[:'on_cancel']
      end
    end
    ...
    def hash
      [id, perform_deletion, concurrency_id, is_removed, display_as, is_completed, is_cancelled, on_create, on_complete, on_cancel].hash
    end
