  ...
  class GeneratePmWoCommand
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Corrigo::GeneratePmWoCommand` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Corrigo::GeneratePmWoCommand`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'pm_schedule_id')
        self.pm_schedule_id = attributes[:'pm_schedule_id']
      end
    end
    ...
    def hash
      [action_date, work_order_id, comment, status_id, concurrency_id, action_reason_id, pm_schedule_id].hash
    end
