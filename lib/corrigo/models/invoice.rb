  ...
  class Invoice
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Corrigo::Invoice` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Corrigo::Invoice`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, perform_deletion, concurrency_id, currency_type_id, time_set_id, period, group, number, state, po_number, dt_due, gl_account, comments, amt_base, amt_tax, amt_total, is_multiline, tax_transaction_id, is_tax_dirty, line_items, ext_number, dt_posted, is_manually_sent, draft_process_step_id].hash
    end
