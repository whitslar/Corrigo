  ...
  class QuoteOrEstimateItem
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Corrigo::QuoteOrEstimateItem` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Corrigo::QuoteOrEstimateItem`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'parent_type_id')
        self.parent_type_id = attributes[:'parent_type_id']
      end
    ...
    def hash
      [id, perform_deletion, concurrency_id, work_order_id, parent_id, parent_type_id, invoice_item, description, quantity, rate, amount, is_flagged, tax_code, currency_type_id].hash
    end
