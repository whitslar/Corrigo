  ...
  class TimeCardItemAllOf
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Corrigo::TimeCardItemAllOf` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Corrigo::TimeCardItemAllOf`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [comment, work_zone, end_date, system_end_date, start_date, system_start_date, week_start_date, duration, employee, labor_code, latitude_start, latitude_stop, longitude_start, longitude_stop, time_card, type_id, internal_cost_item, work_order, qty, rate, money_id].hash
    end
