  ...
  class UserPayRate
    ...
    def self.openapi_types
      {
        ...
        :'labor_code_id' => :'Integer',
        :'user_id' => :'Integer',
        :'is_default' => :'Boolean',
        :'currencies' => :'Array<UserPayRateMoneyValues>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Corrigo::UserPayRate` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Corrigo::UserPayRate`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'labor_code_id')
        self.labor_code_id = attributes[:'labor_code_id']
      end
    ...
    def hash
      [id, perform_deletion, concurrency_id, labor_code_id, user_id, is_default, currencies].hash
    end
