  ...
  class WoClearFlagsCommand
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Corrigo::WoClearFlagsCommand` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Corrigo::WoClearFlagsCommand`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'flags_to_clear')
        if (value = attributes[:'flags_to_clear']).is_a?(Array)
          self.flags_to_clear = value
        end
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          flags_to_clear == o.flags_to_clear
    end
    ...
    def hash
      [action_date, work_order_id, comment, status_id, concurrency_id, action_reason_id, flags_to_clear].hash
    end
