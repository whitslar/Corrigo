  ...
  class WoOnSite
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Corrigo::WoOnSite` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Corrigo::WoOnSite`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, perform_deletion, concurrency_id, work_order_id, employee_id, time_zone, dt_check_in, in_check_type_id, in_validation_value, in_latitude, in_longitude, in_inv_item_id, in_distance, in_status_id, dt_check_out, out_check_type_id, out_validation_value, out_latitude, out_longitude, out_inv_item_id, out_distance, out_status_id].hash
    end
