...
module Corrigo
  class WoVerification
    ...
    attr_accessor :wo_rating_id

    attr_accessor :comment

    attr_accessor :edited_date
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Corrigo::WoVerification` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Corrigo::WoVerification`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'wo_rating_id')
        self.wo_rating_id = attributes[:'wo_rating_id']
      end
      ...
      if attributes.key?(:'edited_date')
        self.edited_date = attributes[:'edited_date']
      end
    ...
    def hash
      [id, perform_deletion, concurrency_id, idx, work_order_id, wo_rating_id, comment, edited_date, actor].hash
    end
